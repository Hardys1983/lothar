$(document).ready(function () {
    ...
    if ($(".mail_tags").length > 0)
        $(".mail_tags").tagsInput({width: '100%', height: 'auto', 'defaultText': 'add email'});
    /* eof tagsinput */

    /* jQuery UI Datepicker { dateFormat: 'dd-mm-yy' }*/
    if ($(".datepicker").length > 0)
        $(".datepicker").datepicker({nextText: "", prevText: "",dateFormat: 'yy-mm-dd'});
    if ($(".mdatepicker").length > 0)
        $(".mdatepicker").datepicker({numberOfMonths: 3, nextText: "", prevText: ""});
    ...
    if ($(".datetimepicker").length > 0)
        $(".datetimepicker").datetimepicker({nextText: "", prevText: ""});
    /* EOF Datetimepicker */
    ...
    if ($("table.sortable_order").length > 0)
        $("table.sortable_order").dataTable({"iDisplayLength": 5, "aLengthMenu": [5, 10, 25, 50, 100], "sPaginationType": "full_numbers", "aoColumns": [{"bSortable": false}, null, null, null, null]});

    if ($('.sortable_mechanic_activites').length > 0) {
        $('.sortable_mechanic_activites').dataTable();
    }

    if ($("table.sortable").length > 0)
        $("table.sortable").dataTable({"iDisplayLength": 5, "aLengthMenu": [5, 10, 25, 50, 100], "sPaginationType": "full_numbers", "aoColumns": [{"bSortable": false}, null, null, null, null, null, null]});
    /* eof Datatables */
