...
<?php if (isset($operation->operation_id)) { ?>
    <div class="row">
        <div class="col-md-12">
            <div class="block">
                <a href='<?= site_url("Vehicular_reception/new_observation/$operation->operation_id") ?>' class="btn btn-primary btn-clean"><i class="fa fa-plus"></i> <?= translate('add') ?></a> 
            </div>
        </div>
    </div>
<?php } ?>
...
<script type="text/javascript">
    function erase(id) {
        $("#btn-delete").attr("href", "<?= site_url("Vehicular_reception/erase_observation") ?>" + "/" + id);
        $("#confirm-delete").modal('show');
    }

    function update(id) {
        window.location.href = "<?= site_url('Vehicular_reception/update_observation') ?>" + "/" + id;
    }
